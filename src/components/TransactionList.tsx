...
export const TransactionList: React.FC<TransactionListProps> = ({
  ...
  const totalIncome = transactions
    .filter(t => t.type === 'income' || t.type === 'estimated_income')
    .reduce((sum, t) => sum + t.amount, 0);

  const totalExpense = transactions
    .filter(t => t.type === 'expense' || t.type === 'estimated_expense')
    .reduce((sum, t) => sum + t.amount, 0);
      ...
      <div className="p-4 md:p-6 border-b border-gray-200">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="text-right">
            <div className="text-xs md:text-sm text-gray-500 mb-1">筛选总计</div>
            <div className={`text-lg md:text-2xl font-bold ${
              netTotal >= 0 ? 'text-green-600' : 'text-red-600'
            }`}>
              {netTotal >= 0 ? '+' : ''}¥{netTotal.toLocaleString('zh-CN', { minimumFractionDigits: 2 })}
            </div>
            <div className="text-xs text-gray-500 mt-1 space-y-0.5">
              <div className="text-green-600">收入: ¥{totalIncome.toLocaleString('zh-CN', { minimumFractionDigits: 2 })}</div>
              <div className="text-red-600">支出: ¥{totalExpense.toLocaleString('zh-CN', { minimumFractionDigits: 2 })}</div>
            </div>
          </div>
        </div>
      </div>
